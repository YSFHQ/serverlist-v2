    ...
    <table class="table table-hover" id="server-list">
        ...
        @foreach ($servers as $server)
            <tr class="{{ $server->status == 'online' ? 'success' : 'danger' }}">
                ...
                <td class="name">
                    <a href="{{ route('map', ['id' => $server->id]) }}"><img src="img/flags/{{ strtolower($server->country) }}.gif" title="{{ strtoupper($server->country) }} - Distance to the server: 9878km"></a>
                    <a href="{{ route('server.show', ['id' => $server->id]) }}" title="owner: {{ $server->owner }} | YS version: 20110207">{{ $server->name }}</a>
                    @if ($server->ip==Request::ip())
                    (<a href="{{ route('server.edit', ['id' => $server->id]) }}">Edit</a>)
                    @endif
                </td>
